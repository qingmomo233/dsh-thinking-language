 ...
 * the id has none.
 *
 * The label is the endonym alone — the exact string the picker shows, so
 * deriving the picker from the schema cannot change what the user reads. `auto`
 * deliberately gets no label: the row renders it through the locale dictionary
 * (`lang.auto`), and leaving it unlabelled keeps the schema entry identical to
 * what it was before.
 *
 * @param id - a catalog id or {@link THINKING_LANGUAGE_DEFAULT}.
 * @returns the endonym, or undefined.
 */
export function languageLabel(id) {
	const meta = catalogEntry(id);
	return meta === undefined ? undefined : meta.native;
}
...
export function describeLanguage(language) {
	if (language === undefined || language === THINKING_LANGUAGE_DEFAULT) return `${THINKING_LANGUAGE_DEFAULT} (follow the system)`;
