window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * The two other services the row needs are acquired through their own
		 ...
		 * before `settingsScope`, so this plugin's apply runs before the locale
		...
		function reportRowAvailability(ctx) {
			...
			const timer = setTimeout(() => {
				const missing = ["settingsScope", "locale"].filter((service) => ctx.get(service) === undefined);
				if (missing.length === 0) return;
				log(ctx, `the ${missing.join(" and ")} service is unavailable; the settings row is disabled — the /thinking-language command still works`);
