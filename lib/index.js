...
function apply(ctx) {
	...
	ctx.inject(["commands", "settings"], (commandsCtx) => {
		const handler = async (invocation) => {
			...
			return {
				kind: "success",
				text: `Thinking language set to ${describeLanguage(parsed.id)}. It applies to new sessions; existing sessions keep their current prompt.`
			};
		};
...
//#endregion
export { THINKING_LANGUAGE_DEFAULT, THINKING_LANGUAGE_FIELD, THINKING_LANGUAGES, THINKING_NAMESPACE, ThinkingLanguageSettingsSchema as Config, apply, currentLanguage, describeLanguage, inject, name, thinkingInstruction, usageLine };
