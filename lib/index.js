...
/**
 * dsh-plugin-thinking-language — switch the language of the agent's
...
/** Durable settings schema; the schema default is `auto` (no instruction). */
...
function describeLanguage(language) {
	if (language === void 0 || language === THINKING_LANGUAGE_DEFAULT) return `${THINKING_LANGUAGE_DEFAULT} (follow the model)`;
...
/**
 * Compose the per-message dynamic reminder for one language id; `auto`
 * yields "". Registered as a prompt context, it is appended right after the
 * user's message on every model step, so the model re-reads it at each call
 * and a language switch takes effect on the very next call — no restart.
 */
...
function usageLine() {
	return `Usage: /thinking-language <id> — ids: ${THINKING_LANGUAGE_IDS.join(", ")} (or "auto" to follow the model)`;
}
/**
...
function apply(ctx) {
	...
	ctx.inject(["systemPrompt", "settings"], (promptCtx) => {
		promptCtx.systemPrompt.section({
			...
			order: 85,
			text: () => thinkingInstruction(currentLanguage(promptCtx.get("settings")))
		});
		...
		promptCtx.systemPrompt.context({
			...
			order: 1000,
			text: () => thinkingReminder(currentLanguage(promptCtx.get("settings")))
		});
	});
...
//#endregion
export { THINKING_LANGUAGE_DEFAULT, THINKING_LANGUAGE_FIELD, THINKING_LANGUAGES, THINKING_NAMESPACE, ThinkingLanguageSettingsSchema as Config, apply, currentLanguage, describeLanguage, inject, name, thinkingInstruction, thinkingReminder, usageLine };
